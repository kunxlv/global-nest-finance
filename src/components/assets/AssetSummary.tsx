...
export default function AssetSummary({ assets, isLoading }: AssetSummaryProps) {
  ...
  const assetsByType = assets.reduce((acc, asset) => {
    ...
    acc[type].count += 1;
    acc[type].value += convertToDisplayCurrency(
      Number(asset.valuation),
      asset.currency as CurrencyCode
    );
    return acc;
  ...
  const distribution = Object.entries(assetsByType)
    .map(([type, data]) => ({
      type,
      value: data.value,
      count: data.count,
        ...
        <div className="bg-card rounded-2xl p-6 border border-border/50">
          ...
          <div className="h-3 rounded-full overflow-hidden flex bg-muted mb-4">
            {distribution.map((item, index) => (
              <div
                key={item.type}
                className={cn(ASSET_COLORS[item.type], "transition-all duration-500")}
