...
export default function CashFlow() {
  ...
  const fetchData = async () => {
    ...
    else setIncomeStreams(incomeRes.data || []);

    if (bankRes.error) toast.error("Failed to load bank accounts");
    else setBankAccounts(bankRes.data || []);
  ...
  const deleteIncomeStream = async (id: string) => {
    ...
    if (error) toast.error("Failed to delete income stream");
    else {
  ...
  const deleteBankAccount = async (id: string) => {
    ...
    if (error) toast.error("Failed to delete bank account");
    else {
                    ...
                    {bankAccounts.map((account) => (
                      <div key={account.id} className="relative group">
                        <BankAccountCard
                          ...
                          balance={formatCurrency(Number(account.balance), account.currency as CurrencyCode)}
